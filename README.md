...
- **サブモード**（ツールバーのセグメント切替）
  - **階層**（v0.5.26 で全面再設計） — **ファイルツリー / Obsidian アウトライン風の縦積み + インデント + ▶/▼ 折りたたみトグル**。上位 2 階層まで既定展開。旧カード階層の横スクロール地獄を解消。VS Code テーマ追従。
...
- **ルート絞り込み**（v0.5.26）— ツールバーの **ルート** セレクタで最上位エージェント（`director` / `Curtain_leader` / `ponta` 等）配下だけを表示。複数プロジェクトが混在してカオスなときにピンポイントで俯瞰。設定 `claudeManager.orgChart.defaultRoot` で永続化
- **グローバル除外**（v0.5.26 で復活）— `parentAgent` 未設定のグローバル汎用エージェント（`qa` / `doc-writer` / `researcher` 等）は組織図の全モードから除外（既定）。ツールバーの **グローバルも表示** トグル or 設定 `claudeManager.orgChart.showGlobal` で ON にできる
- **線視認性 + 親→子矢印**（v0.5.26）— グラフモードの親子エッジを太く・明るく、方向がわかる小さな三角矢印を追加。連携（金色点線）と形状・色で区別
- **設定**: `claudeManager.orgChart.defaultMode`（`graph`（既定） / `tree` / `group`）、`claudeManager.orgChart.hideOtherProjects`、`claudeManager.orgChart.showGlobal`、`claudeManager.orgChart.defaultRoot`
...
- **v0.5.30** — エージェント一覧の起動高速化（2 段レンダリング: 一覧を即表示 → 他プロジェクトのセッションタイトル解決は非同期で追いつく、`agentFileManager` キャッシュ 2s → 10s、多数エージェント環境で体感数十ms）
- **v0.5.29** — 全『Claude で開く』ボタンの新ウィンドウ挙動を統一（共通ヘルパー `openInClaudeHelper.ts` に一本化、エージェント/会話一覧/ライブ状態/オーケストレーション/組織図/会話ビューワーの 7 経路すべてで対象フォルダがワークスペース外なら自動で新ウィンドウ起動）
- **v0.5.27** — エージェント基本情報にフォルダパス（クリックで OS エクスプローラで開く）+ 「Claude で開く」で対象フォルダが現ワークスペース外なら新しい VS Code ウィンドウを自動起動（設定 `agent.openInNewWindowWhenFolderMismatch`、既定 ON）
- **v0.5.26** — 組織図の整理（グローバルエージェントの混入バグ修正・ルート絞り込みセレクタ・階層モードを縦型インデントツリーに全面再設計・折りたたみ対応・親→子矢印と線太さ UP で視認性改善）
