...
- **未知の frontmatter キーは保持**: Claude Code 本体が後から追加したキー（2.1.271 の `omitClaudeMd`、
  ほか `skills` / `hooks` / `mcpServers` / `color` など）を手で書いていても、フォームから保存して消えることはありません。
  CSM が編集対象にしていないキーは原文のまま末尾に戻します。
- **公式拡張の「セッション自動アーカイブ」（2.1.265〜、既定 14 日）**: アーカイブは公式拡張の一覧上の表示状態で、
  **会話の実体（JSONL）は移動も削除もされません**。CSM の紐づけ・バックアップ・「Claude で開く」には影響しません。
  「CSM には出るのに Claude Code のセッション一覧に無い」ときは、一覧下部の **Archived sessions** を確認してください
  （設定 `claudeCode.archiveInactiveSessions`。`0` で無効）。
  なお実体が消えるのは別機構の `cleanupPeriodDays`（既定 30 日）で、こちらは引き続き CSM のバックアップが保険になります。
- **公式拡張の「agent map」（2.1.269〜）**: フッターの「N agents」から、**そのセッションの**サブエージェントを
  カード表示する公式機能です（停止・読み取り専用トランスクリプト付き）。単位が「1 セッション」なのに対し、
  CSM の **⚪ 一時実行** は「1 エージェント」単位で、複数の呼び出し元セッションにまたがる依頼履歴を並べます。
