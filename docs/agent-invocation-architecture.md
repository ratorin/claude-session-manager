...
## 9. Claude Code 2.1.265〜2.1.274 の影響（2026-09-17 追記）

本文書の論点に関わる本体側の変更。

### 9.1 G4（セッション肥大化）の圧力は下がった

| 版 | 変更 | 本文書への影響 |
|---|---|---|
| 2.1.267 | 5 MB を超えるセッションの再開で、並列ツール呼び出しとその hook 出力が**欠落していた不具合を修正** | 経路 B（`--resume`）で大きなセッションを再開すると文脈が一部抜けていた、ということ。肥大化の実害は「遅い」だけではなかった |
| 2.1.267 | Bash 呼び出しの多いセッションで `--resume` の初回描画を高速化 | G4 の「起動が重い」を緩和 |
| 2.1.268 | `--continue` / `--resume` が SessionStart hook を待たずに会話を表示。最初のメッセージで**トランスクリプト全体を読み直さなくなった** | 同上。CSM は SessionStart hook（役割注入）を入れているので直接効く |

→ 「肥大化しているから経路 A（使い捨て）に逃げる」動機は弱まった。ただし 88 MB〜527 MB 級では依然として重いので、
R5 本体（アーカイブ分割 / サマリ引き継ぎ）の必要性は残る。**CLI を 2.1.268 以降に上げることが G4 の最も安い緩和策**。

### 9.2 公式拡張に「agent map」が入った（2.1.269）

VS Code 拡張のフッター「N agents」から、そのセッションのサブエージェントをカード表示できる
（停止・読み取り専用トランスクリプト付き）。G1「経路 A が見えない」に対する本体側の部分的な回答。

ただし単位が違う:

| | 公式 agent map | CSM ⚪ 一時実行 |
|---|---|---|
| 単位 | 1 セッション | 1 エージェント |
| 答えられる問い | 「このセッションは今だれを動かしているか」 | 「このエージェントは過去にだれから何を頼まれたか」 |
| 起点 | 呼び出し元セッションを開いている必要がある | エージェント一覧から辿れる |

§1 の事故（ユーザーが CSM で `Craft_sv` を開いて「会話が無い」）は後者の問いなので、R3/R4 は引き続き必要。

### 9.3 公式拡張の「セッション自動アーカイブ」（2.1.265、既定 14 日）

拡張 2.1.273 の実装を確認した結果、アーカイブ状態は拡張の `globalState`（キー `hiddenSessionIds`）に
セッション ID の配列として持つだけで、**JSONL 実体は移動も削除もされない**。

- `agentSessions` の紐づけ・`--resume`・CSM のバックアップには**影響なし**
- 紐づけセッションは 14 日触らないと公式一覧では「Archived sessions」に畳まれるため、
  「CSM にはあるのに公式一覧に無い」という見え方のズレは起きうる（README に注記済み）
- 実体が消えるのは別機構の `cleanupPeriodDays`。こちらは従来どおり

### 9.4 `omitClaudeMd`（2.1.271）と、CSM が未知キーを消していた問題

エージェント frontmatter に `omitClaudeMd` が追加された（サブエージェントを CLAUDE.md 無しで走らせる）。
これを機に確認したところ、CSM の `buildFrontmatter` は**決め打ちのキーしか書き出さず、
フォームから保存すると未知キーが黙って消える**実装だった。105 件の定義を走査した時点では該当 0 件で実害は無かったが、
`omitClaudeMd` を書いた瞬間に発症する状態。未知キーを原文のまま保持するよう修正した（v0.6.1）。

運用上の注意: `omitClaudeMd: true` は経路 A（Agent ツール）で走るサブエージェント向け。
このリポジトリの運用では CLAUDE.md に「使い捨てである旨を明示する」「`CSM_SUMMARY` を出す」等の規律が入っているため、
**部署エージェントに付けると規律ごと外れる**。付けるなら `code-reviewer` のような汎用レビュー系に限るのが安全。

### 9.5 クロスセッション通信の既読誤認（2.1.271）

`SendMessage` の結果が「相手が読んだ」ように読める表現だったのが修正された。受信側の permission-mode ポリシーで
メッセージが保留されることがあり、その場合 headless の送信側には配送通知が返る。
→ **`SendMessage` が成功しても相手が読んだとは限らない。** 返事が要るなら `notify_when_idle` で待つ（`c:/xampp/CLAUDE.md` に反映）。
