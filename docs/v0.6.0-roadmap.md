...
> **基礎資料**（v0.5.31 で `docs/archive/` へ退避済み）:
> - [`archive/v0.5.x-cc-compat-qa.md`](./archive/v0.5.x-cc-compat-qa.md)（健全度 78/100、R-1〜R-5）
> - [`archive/v0.5.x-feature-audit.md`](./archive/v0.5.x-feature-audit.md)
> - [`archive/v0.5.x-workflows-integration.md`](./archive/v0.5.x-workflows-integration.md)
...
| 組織図（親子・連携の可視化） | 無し（テキスト frontmatter のみ） | `orgChartPanel`（Canvas 自前実装、v0.5.23〜） | **CSM 独占領域** |
...
| 対象 | 工数 | 優先度 | 根拠 |
|---|---|---|---|
| オーケストレーションタブ Phase 2（Webview ガント／ツリー） | 5.25d | **P1** | `archive/v0.5.x-workflows-integration.md` §11.4。実データ（kind）で価値が出る |
| オーケストレーションタブ Phase 3（組織図ライブオーバーレイ） | 2.0d | **P1** | 同上。実データで動く |
| メニュー再設計 v2（claudeAgents/Favorites の重複登録解消） | 3.0d | **P1** | `archive/v0.5.x-menu-redesign-2.md` を完遂 |
...
| 組織図 (Canvas 自前実装、v0.5.23〜。旧 Cytoscape + ELK は撤去) | ◎ | 独自価値、Phase 3 で強化 |
...
- 2026-07-17 v0.5.31 QA 対応: 基礎資料 3 本の相対リンクを `archive/` 配下へ更新、組織図の実装記述を Canvas 自前実装（v0.5.23〜）に修正
